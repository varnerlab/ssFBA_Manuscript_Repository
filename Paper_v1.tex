...
We compared the theoretical optimum solutions with an experimentally constrained case which suggested CFPS retained a growth-like operational memory that led to over consumption of glucose, and low energy efficiency due to suboptimal oxidative phosphorylation activity.
...
From this, higher carbon number proteins typically had lower productivity rates and carbon yields than that of the lower carbon number proteins; whereas energy efficiency was independent of protein size.
Our model correlations were independently validated with a protein not in our original data set.
We compared the theoretical optimum solutions of ssFBA with an experimentally constrained case which suggested CFPS retained a growth-like operational memory that led to over consumption of glucose, and over production of ATP.
...
In particular, we calculated the transcription rate using effective promoter models in combination with a maximum transcription rate estimated from literature.
...
The translation rate has been identified as the rate-limiting step for cell-free protein synthesis \cite{2005_underwood_biotech,2014_li_PlosOne}, which was confirmed by the translation rate flux always hitting the upper bound in the ssFBA simulation, regardless of protein.
...
To more accurately describe actual metabolic flux, we constrained the feasible solution space with experimental measurements
...
The largest discrepancy was in oxidative phosphorylation,
where the experimental system relied heavily on \textit{cyd} rather than \textit{cyo} which was less efficient for energy generation.
In addition, the experimental system had a high flux through the \textit{zwf} yielding an abundance of NADPH used for amino acid biosynthesis.
...
Next, folate, purine, and pyrimidine metabolism along with amino acid biosynthesis were inactive in the optimal system, but active in the experimental system.
In particular, the experimental system had high alanine and glutamine biosynthetic flux (both accumulated in the media),
while there was no accumulation of amino acids in the optimal simulations.
...
Thus, it is unclear which substrate (glucose or amino acids) was used to power CFPS and may in fact be a combination of both.
...
We thank Che-Hsiao Shih (Purdue University) for identification of DNA and protein sequences.
