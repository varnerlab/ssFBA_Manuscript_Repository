...
Constraints based methods such as FBA, metabolic flux analysis (MFA) \cite{WIECHERT2001195}, and network decomposition approaches such as elementary modes \cite{Schuster:2000aa} or extreme pathways \cite{Schilling:2000aa} model intracellular metabolism using biochemical stoichiometry and other constraints such as thermodynamical feasibility \cite{Henry:2007,Hamilton:2013} under pseudo steady state conditions. 
These constraints based approaches have predicted productivity \cite{Varma01101994, BTPR:BTPR2132}, yield \cite{Varma01101994}, mutant behavior \cite{Edwards2000}, and growth phenotypes \cite{Brien693} of arbitrary complexity, including genome scale networks, using linear programming\cite{Covert:2004aa}.  
...
Stoichiometric reconstructions have been expanded to include metabolism integrated with detailed gene expression (ME-Model) \cite{Allen:2003aa, Brien693}, and protein structures (GEM-PRO) \cite{Zhang1544,Chang1220} which has increased the scope of simulating genetic and environmental perturbations.
...
Constraints based methods are great tools to estimate the performance of metabolic networks with none to very few parameters. 
However, constraints based methods are typically used to model \textit{in~vivo} processes and have not been applied to cell-free metabolism.
...
The mean productivity was inversely proportional to the protein size and varied between 1 and 12 $\mu$M/h for the proteins sampled (Fig.~\ref{fig:Prof}A-B).
...
\subsubsection{Yield}
The mean carbon yield was inversely proportional to protein size and varied between 40-57\% for the proteins sampled (Fig.~\ref{fig:Prof}E-F).
...
A single trendline was formulated for the cases where amino acids were supplied in the media since they had similar performance, while another trendline was formulated for when amino acids were not available (Fig.~\ref{fig:Yield}F).
